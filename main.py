...
with open('config.json') as json_file:
    ...
    TOKEN = data["token"]
    PREFIX = data["prefix"]
    ROLES = data["permission_roles"] #To find Role ID right click your role and click Copy ID (Make sure you have developer mode on) http://j6y.club/files/Discord_XvczUr3o78.png http://j6y.club/files/Discord_ZqPXEyxTSa.png put the Role ID in the config.json file.
    API_KEY = data["api_key"] #To find sellix api key go to user Settings > Security https://j6y.club/files/opera_8sCiBBarJJ.png put the api key in the config.json file.
    CUSTOMER_ROLE = data["customer_role"] #To find Role ID right click your role and click Copy ID (Make sure you have developer mode on) http://j6y.club/files/Discord_XvczUr3o78.png http://j6y.club/files/Discord_ZqPXEyxTSa.png put the Role ID in the config.json file.
...
async def update():
    ...
    with open('config.json') as json_file:
        data = json.load(json_file)
        restock_channel = data["restock_channel"] #To find restock Channel ID right click the channel you want and click copy id (Make sure you have Developer mode on) put the Channel ID in the config.json file inside the brackets that say RestockChannelIDhere http://j6y.club/files/Discord_ZqPXEyxTSa.png http://j6y.club/files/Discord_XvczUr3o78.png
    print(Style.RESET_ALL + Fore.YELLOW + "> Updating...")
        ...
        if product["uniqid"] in stock_list:
            if stock_list[product["uniqid"]] != int(product["stock"]):
                ...
                if int(product["stock"]) == 1:
                    ...
                    embed = discord.Embed(title="Stock Empty", description=product["title"] + " is empty on stock",
            ...
            stock_list[product["uniqid"]] = int(product["stock"])
    print(Style.BRIGHT + Fore.GREEN + "> Update finished ")
...
async def help(ctx):
    ...
    embed.add_field(name=PREFIX+"verify or claim <order_id>", value="Adds customer role to an user", inline=False)
    if has_permissions(ctx):
        embed.add_field(name=PREFIX+"order <order_id>", value="Prints all information about an order", inline=False)
        embed.add_field(name=PREFIX+"replace <order_id> <amount>", value="Replace account from sellix stock", inline=False)
    await ctx.send(embed=embed)


@client.command(name="verify", aliases=["claim"])
async def verify(ctx, order_id):
    ...
    embed = discord.Embed(title="Success", description="You should receive the customer role now!",
...
async def checkorder(ctx, order_id):
    ...
    embed = discord.Embed(title="Order info", description="Order ID: "+order_information["uniqid"], color=discord.Color.purple())
...
async def on_ready():
    printBanner()
    print(Style.BRIGHT + Fore.CYAN + "> Command Prefix is " + PREFIX)
    print(Style.BRIGHT + Fore.CYAN + '> Logged in as {0.user}'.format(client))
...
def printBanner():
    ...
    print(Fore.MAGENTA + "╚══════╝╚══════╝╚══════╝╚══════╝╚═╝╚═╝  ╚═╝    ╚═════╝  ╚═════╝    ╚═╝   ")
    print(Fore.MAGENTA + "                                                                         ")
    print(Fore.CYAN + "> Started Sellix Bot by Nergon#4972")
    print(Style.BRIGHT + Fore.MAGENTA + "> Forked by paige<3#0001")
